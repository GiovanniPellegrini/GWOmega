...
def create_C_hat_triangular_RD(A_s, k_peak, sigma,r, n_noise, T_obs, N_seg, C_hat_filename="data/triangular/C_hat_triangular_RD.h5", parameters_filename="data/triangular/parameters_triangular_RD.txt"):
    """
    Create data files for the A and E channels of the Einstein Telescope in a triangular configuration with correlated noise in RD
    ...
    gamma_matrix= gamma_aet(f_values)[:2]
    f_pivot = 2.75
    N_func= N_auto_interp("data/ET_Sh_coba.txt")
    N_auto=N_func(f_values)
    N_amplitude=float(N_func(f_pivot))
    N_matrix= N_aet(f_values, N_auto, f_pivot, N_amplitude, r, n_noise)[:2]
    S_0= S0(f_values)
    ...
    mean= gamma_matrix * Omega_gw+ N_matrix/S_0
    var= mean**2/N_seg
...
def create_C_hat_triangular_eMD(A_s, k_max, eta_R, r, n_noise, T_obs, N_seg, C_hat_filename="data/triangular/C_hat_triangular_eMD.h5", parameters_filename="data/triangular/parameters_triangular_eMD.txt"):
    """
    Create data files for the A and E channels of the Einstein Telescope in a triangular configuration with correlated noise in RD
    We compute C_hat directly from the theoretical mean and variance, without generating individual signal and noise realizations.
    """
